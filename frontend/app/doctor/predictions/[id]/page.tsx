...
export default function PredictionDetailPage() {
    ...
    const loadPredictionDetail = async () => {
        try {
            ...
            setReviewNotes((data as PredictionDetail).doctor_notes || '');
        } catch (err: any) {
    ...
    const handleReview = async (approve: boolean) => {
        try {
            setSubmitting(true);
            setError('');
            await doctorApi.reviewPrediction(predictionId, reviewNotes, approve);
            alert(approve ? 'Prediction approved successfully' : 'Prediction rejected successfully');
            router.push('/doctor/dashboard');
        } catch (err: any) {
            const errorMessage = err.response?.data?.detail || err.message || 'Failed to submit review. Please try again.';
            setError(errorMessage);
            console.error('Error reviewing prediction:', err);
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <div className="min-h-screen bg-gray-50">
                <div className="max-w-4xl mx-auto py-8 px-4">
                    <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
                        ...
                        <Link href="/doctor/dashboard" className="text-blue-600 hover:text-blue-800 font-medium">
                            ← Back to Dashboard
                        </Link>
                    </div>
                </div>
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <header className="bg-white shadow">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <Link href="/doctor/dashboard" className="text-blue-600 hover:text-blue-800 font-medium mb-4 block">
                        ← Back to Dashboard
                    </Link>
                    <h1 className="text-2xl font-bold text-gray-900">Prediction Review</h1>
                </div>
            </header>
                ...
                <div className="bg-white shadow rounded-lg p-8 space-y-8">
                    ...
                    {prediction.status === 'pending' && (
                        <div className="bg-blue-50 border-2 border-blue-200 p-8 rounded-lg">
                            <h2 className="text-xl font-semibold text-gray-900 mb-6">Doctor Review</h2>
                            ...
                            <div className="mb-6">
                                ...
                                <textarea
                                    ...
                                    rows={6}
                                    placeholder="Add your medical review and notes here..."
                                />
                            </div>
                            ...
                            <div className="flex gap-4">
                                <button
                                    ...
                                    disabled={submitting}
                                    className="flex-1 px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 disabled:bg-gray-400 transition"
                                >
                                    {submitting ? 'Approving...' : '✓ Approve Prediction'}
                                </button>
                                <button
                                    onClick={() => handleReview(false)}
                                    disabled={submitting}
                                    className="flex-1 px-6 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 disabled:bg-gray-400 transition"
                                >
                                    {submitting ? 'Rejecting...' : '✗ Reject Prediction'}
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {prediction.status !== 'pending' && (
                        <div className={`${prediction.status === 'approved' ? 'bg-green-50 border-2 border-green-200' : 'bg-red-50 border-2 border-red-200'} p-8 rounded-lg`}>
                            <h2 className={`text-xl font-semibold mb-4 ${prediction.status === 'approved' ? 'text-green-900' : 'text-red-900'}`}>
                                Review Status: {prediction.status?.charAt(0).toUpperCase() + prediction.status?.slice(1)}
                            </h2>
                            {prediction.doctor_notes && (
                                <div className={`${prediction.status === 'approved' ? 'text-green-700' : 'text-red-700'}`}>
